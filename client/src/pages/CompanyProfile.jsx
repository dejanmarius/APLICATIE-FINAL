            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel
                  className={`${
                    theme === "dark"
                      ? "w-full max-w-md transform overflow-hidden rounded-2xl bg-slate-700 p-6 text-left align-middle shadow-xl transition-all"
                    ...
                    <div className="mt-4">
                      {isLoading ? (
                        ...
                        <CustomButton
                          ...
                          containerStyles={`${
                            theme === "dark"
                              ? "inline-flex justify-center rounded-md border border-transparent bg-slate-500 px-8 py-2 text-sm font-medium text-white hover:bg-white hover:text-black focus:outline-none"
                              : "inline-flex justify-center rounded-md border border-transparent bg-blue-600 px-8 py-2 text-sm font-medium text-white hover:bg-[#1d4fd846] hover:text-[#1d4fd8] focus:outline-none"
                          }`}
                          title={"Submit"}
                        />
                      )}
      ...
      <div className="container mx-auto p-5 ">
        <div className="">
          <div className="w-full flex flex-col md:flex-row gap-3 justify-between">
            ...
            <h2 className="dark:text-white text-gray-600 text-xl font-semibold">
              
              Welcome, to {info?.name} profile page
            </h2>)}

            {user?.user?.accountType === undefined &&
              info?._id === user?._id && (
                <div className="flex items-center ml-auto py-5 md:py-0 gap-4">
                  <CustomButton
                    onClick={() => setOpenForm(true)}
                    iconRight={<FiEdit3 />}
                    containerStyles={`py-1.5 px-3 md:px-5 focus:outline-none bg-blue-600  hover:bg-blue-700 text-white rounded text-sm md:text-base border border-blue-600 dark:bg-slate-600 dark:border-white dark:hover:bg-white dark:hover:text-black`}
                  />
                  ...
                  <Link to="/upload-job">
                    <CustomButton
                      title="Upload Job"
                      iconRight={<FiUpload />}
                      containerStyles={`text-blue-600 py-1.5 px-3 md:px-5 focus:outline-none  rounded text-sm md:text-base border border-blue-600 dark:bg-slate-600 dark:border-white
          ...
          <div className="w-full flex flex-col md:flex-row justify-start md:justify-between mt-4 md:mt-8 text-sm">
            ...
            <div className="flex flex-col items-center mt-10 md:mt-0   dark:text-white ">
              <span className="text-xl">{info?.jobPosts?.length}</span>
              <p className="text-blue-600   dark:text-white ">Job Post</p>
            </div>
          </div>
        </div>
        <div>
        ...
        <div className="w-full mt-20 flex flex-col gap-2   dark:text-white ">
          <p>Jobs Posted</p>
